...
class Funileiros(CRUD):
    ...
    def abrir(cls):
        cls.objetos = []
        try:
            with open("funileiros.json", mode="r") as arquivo:
                s = json.load(arquivo)
                for dic in s:
                    c = Funileiro(dic["id"], dic["nome"], dic["email"], dic["fone"], dic["senha"])
    ...
    @classmethod
    def salvar(cls):
        with open("funileiros.json", mode="w") as arquivo:
